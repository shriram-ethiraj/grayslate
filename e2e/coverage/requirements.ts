...
export const REQUIREMENTS: readonly Requirement[] = [
  ...
  { id: "file.menu-open.multiple", area: "file-lifecycle", behavior: "File Open accepts an ordered multi-selection, opens every valid file, and keeps the final file in the receiving window.", coverage: "e2e" },
  { id: "file.open.active-feedback", area: "file-lifecycle", behavior: "Opening the active file again from the sidebar, file picker, new-window picker, or external input reports that it is already open without reloading or prompting.", coverage: "e2e" },
  ...
  { id: "file.drop.multiple", area: "file-lifecycle", behavior: "Dropping several valid files tracks and opens all of them, keeping the last file in the receiving window.", coverage: "e2e" },
  ...
  { id: "file.drop.unsaved-guard", area: "file-lifecycle", behavior: "A dropped file can be cancelled, opened in a new window, or opened here after discarding changes.", coverage: "e2e" },
  { id: "file.drop.all-owned-dirty", area: "file-lifecycle", behavior: "A dirty external document is preserved without prompting when the final dropped target already has an owner.", coverage: "e2e" },
  { id: "file.drop.dialog-queue", area: "file-lifecycle", behavior: "A file dropped while an app dialog is open waits until that dialog closes.", coverage: "e2e" },
  { id: "file.drop.active-noop", area: "file-lifecycle", behavior: "Dropping the active document does not reload it or prompt over unsaved edits.", coverage: "e2e" },
  { id: "file.drop.native-overlay", area: "file-lifecycle", behavior: "Dragging native files over the workspace shows and clears the full drop overlay without changing pane geometry.", coverage: "manual", reason: "WebDriver cannot synthesize Tauri's OS-owned native drag lifecycle; verify the real pointer and compositor path on each packaged desktop target.", owner: "release reviewer" },
  ...
  { id: "file.guard.cancel-and-discard", area: "file-lifecycle", behavior: "Leaving a dirty local document supports Cancel, Discard, and creating a new slate window without losing edits.", coverage: "e2e" },
  ...
  { id: "file.copy-path", area: "file-lifecycle", behavior: "Copy path puts the document's full path on the clipboard and confirms success.", coverage: "e2e" },
  ...
  { id: "file.open.reservation-cancel", area: "file-lifecycle", behavior: "Cancelling a superseded document-open reservation cannot replace the active document.", coverage: "rust", reason: "Covered by window::tests::cancelled_reservation_does_not_replace_the_active_document." },
  ...
  { id: "editor.clipboard.copy-document", area: "editor", behavior: "Copy places the whole document on the system clipboard and confirms success.", coverage: "e2e" },
  ...
  { id: "csv.copy", area: "csv", behavior: "Copying yields correctly escaped CSV, commits any open edit first, and confirms success.", coverage: "e2e" },
  ...
  { id: "markdown.copy", area: "markdown", behavior: "The preview context menu copies the selection, selects all, and confirms success.", coverage: "e2e" },
  ...
  { id: "shell.updates.install", area: "settings-shell", behavior: "Installing an available update obtains approval from every window, locks further edits, flushes every slate, and invokes the installer only when all documents are safe.", coverage: "manual", reason: "Needs a signed update artifact and a real update server; verify dirty-local Save, Discard, and Cancel choices across multiple windows before release.", owner: "release reviewer" },
  ...
  { id: "shell.tooltips.window-lifecycle", area: "settings-shell", behavior: "Transient tooltips close when their native window deactivates and stay closed until fresh user engagement.", coverage: "e2e" },
