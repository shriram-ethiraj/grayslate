---
name: code-review
description: Reusable review checklist for auditing Grayslate's Svelte 5, TypeScript, CodeMirror, CSV table, and Tauri backend changes, including fail-closed IPC capabilities and CSP/webview hardening.
---
...
- application commands missing from `APP_COMMANDS`, capability grants, or the
  `generate_handler!` registration
- generated permission TOML being edited or committed instead of treating
  `src-tauri/permissions/autogenerated/` as build output
- broad plugin grants such as `core:default`, `dialog:default`,
  `opener:default`, `os:default`, clipboard read, or window destroy
- CSP regressions: null CSP, script inline/eval, broad network/image sources,
  frames, forms, workers, objects, media, or remote Markdown image fetching
- claims that external-navigation blocking, `freezePrototype`, `nosniff`, or a
  restrictive `Permissions-Policy` are implemented without code and packaged
  tests proving them
...
- `src-tauri/src/command_names.rs`
- `src-tauri/build.rs`
- `src-tauri/src/menu/mod.rs`
- `src-tauri/src/window/mod.rs`
- `src-tauri/tauri.conf.json`
- `src-tauri/capabilities/*.json`
- `src-tauri/src/capability_tests.rs`
...
- application IPC added only to `generate_handler!` or only to the capability
- edits under `src-tauri/permissions/autogenerated/`
- `'unsafe-inline'` or `'unsafe-eval'` in `script-src`
- `https:`, `asset:`, or wildcard CSP sources added without a packaged-build
  requirement
...
5. For IPC/config changes, compare `APP_COMMANDS`, `generate_handler!`, the main
   capability, and the regression test as one authorization unit.
6. For UI changes, inspect cleanup paths and flex container chains.
7. For worker or async changes, ask what happens on restart, unmount, cancellation, and error.
8. Prefer one high-signal comment per root cause, with all affected files/lines grouped together.
...
After capability or CSP changes, run the packaged IPC/security E2E and verify
both an intended application command and representative denied plugin commands.
Do not treat browser-only dev tests as proof of the packaged ACL or CSP.
