...
async function main() {
    ...
    assert("Svelte extension", languageDetector.detect("", "App.svelte"), "svelte");
    assert("Vue extension", languageDetector.detect("", "App.vue"), "vue");
    assert("Rust extension", languageDetector.detect("", "main.rs"), "rust");
    assert("Clojure extension", languageDetector.detect("", "core.clj"), "clojure");
    assert("ClojureScript ext", languageDetector.detect("", "app.cljs"), "clojure");
    assert("EDN extension", languageDetector.detect("", "deps.edn"), "clojure");
  ...
  "compilerOptions": {
    ...
    // ════════════════════════════════════════════════════════════════
    // Phase 3 — Component Frameworks (Svelte & Vue)
    // ════════════════════════════════════════════════════════════════

    console.log("\n── Phase 3: Svelte & Vue Detection ──");

    assert("Svelte snippet (block tags)", languageDetector.detect(`
<script lang="ts">
  let count = $state(0);
</script>
...
`), "svelte");
    ...
    assert("Svelte NOT detected as HTML", languageDetector.detect(`
<script>
...
`), "svelte");
    ...
    assert("Vue SFC snippet", languageDetector.detect(`
<template>
  <div v-if="show">
    <input v-model="name" />
...
`), "cpp");
    ...
    assert("Rust code", languageDetector.detect(`
use std::io;
...
`), "rust");
    ...
    assert("Rust with structs & impl", languageDetector.detect(`
#[derive(Debug, Clone)]
pub struct Point {
    x: f64,
    y: f64,
...
`), "rust");
    ...
    assert("Clojure code", languageDetector.detect(`
(ns myapp.core
...
`), "clojure");
    ...
    assert("TS NOT detected as JS", languageDetector.detect(`
interface Config { port: number; host: string; }
const config: Config = { port: 3000, host: "localhost" };
export type Mode = "dev" | "prod";
`), "typescript");

    assert("CSS NOT detected as HTML", languageDetector.detect(`
