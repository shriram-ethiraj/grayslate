...
- **Language Detection:** Uses a Rust-side family-first pipeline in `src-tauri/src/detection/`: extension / filename → shebang → strong structural probes → content family classification → family-gated scoring → rival disambiguation / tree-sitter, with legacy heuristic fallback when v2 abstains. The FE retains only thin sync UI maps (`languageExtensions.ts`, `languageIconMap.ts`) while all content-based detection still goes through IPC.
...
- **Library Sidebar:** Recent-files refresh is backend-driven via `files://recent-updated`, emitted after read/save/rename/delete/duplicate flows. The sidebar suppresses reordering after sidebar-initiated opens and uses quiet refresh + rename-path tracking so the visible list does not jump under the cursor.
