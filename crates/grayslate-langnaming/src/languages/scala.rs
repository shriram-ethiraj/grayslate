...
pub fn definition() -> NamingDefinition {
    NamingDefinition {
        name: "scala",
        extension: "scala",
        extract: Extractor::Custom(extract_scala),
    }
}
...
mod tests {
    ...
    #[test]
    fn scala_companion_object() {
        let src = "case class Order(id: Long, total: Double)\nobject Order {\n  def empty: Order = Order(0, 0.0)\n}";
        let n = name(src).unwrap();
        assert!(n.contains("order"), "companion object: {n}");
    }

    #[test]
    fn scala_implicit_class() {
        let src = "implicit class RichString(val s: String) extends AnyVal {\n  def words: Array[String] = s.split(\" \")\n}";
        let n = name(src).unwrap();
        assert!(n.contains("rich-string"), "implicit class: {n}");
    }
}
