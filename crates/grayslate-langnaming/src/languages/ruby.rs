...
pub fn definition() -> NamingDefinition {
    NamingDefinition {
        name: "ruby",
        extension: "rb",
        extract: Extractor::Custom(extract_ruby),
    }
}
...
RSpec.describe UserService do
  it 'authenticates users' do
    ...
    #[test]
    fn rake_task() {
        let src = "desc 'Compile assets'\ntask :compile_assets do\n  sh 'webpack'\nend";
        let n = name(src).unwrap();
        assert!(n.contains("compile"), "rake task: {n}");
    }
    ...
    fn class_with_inheritance() {
        ...
        let n = name(src).unwrap();
        assert!(n.contains("token-validator"), "class with inheritance: {n}");
    }
}
