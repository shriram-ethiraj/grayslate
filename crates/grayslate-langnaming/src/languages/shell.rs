...
pub fn definition() -> NamingDefinition {
    NamingDefinition {
        name: "shell",
        extension: "sh",
        extract: Extractor::Custom(extract_shell),
    }
}
...
mod tests {
    ...
    fn function_keyword_syntax() {
        ...
        let n = name(src).unwrap();
        assert!(n.contains("deploy-service"), "function keyword: {n}");
    }

    #[test]
    fn description_beats_functions() {
        let src = "#!/bin/bash\n# Rotate and compress old log files\ncompress_logs() {\n  gzip *.log\n}\n";
        let n = name(src).unwrap();
        assert!(n.contains("rotate"), "description P10 > function P7: {n}");
    }
}
