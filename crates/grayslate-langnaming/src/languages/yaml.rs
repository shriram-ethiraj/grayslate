...
pub fn definition() -> NamingDefinition {
    NamingDefinition {
        name: "yaml",
        extension: "yaml",
        extract: Extractor::Custom(extract_yaml_enhanced),
    }
}
...
mod tests {
    ...
    #[test]
    fn kubernetes_service() {
        let src = "apiVersion: v1\nkind: Service\nmetadata:\n  name: auth-service\nspec:\n  ports:\n    - port: 443";
        let n = name(src).unwrap();
        assert!(n.contains("service"), "kind: {n}");
        assert!(n.contains("auth"), "resource name: {n}");
    }

    #[test]
    fn cloudformation_no_description() {
        let src = "AWSTemplateFormatVersion: '2010-09-09'\nResources:\n  MyBucket:\n    Type: AWS::S3::Bucket\nOutputs:\n  BucketArn:\n    Value: !GetAtt MyBucket.Arn";
        let n = name(src).unwrap();
        assert!(n.contains("cloudformation"), "cfn fallback: {n}");
    }
}
