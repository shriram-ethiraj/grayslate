...
pub fn definition() -> NamingDefinition {
    NamingDefinition {
        name: "css",
        extension: "css",
        extract: Extractor::Custom(extract_css),
    }
}

/// CSS naming: extract prominent selectors, @keyframes, and framework markers.
pub(super) fn extract_css(content: &str) -> Option<String> {
    use regex::Regex;
...
mod tests {
    ...
    #[test]
    fn class_selector_extraction() {
        let src = ".login-form {\n  display: flex;\n  flex-direction: column;\n}\n.login-form input {\n  margin: 0.5rem;\n}";
        let n = name(src).unwrap();
        assert!(n.contains("login-form"), "class selector: {n}");
    }

    #[test]
    fn tailwind_detection() {
        let src = "@tailwind base;\n@tailwind components;\n@tailwind utilities;\n";
        let n = name(src).unwrap();
        assert_eq!(n, "tailwind-base");
    }

    #[test]
    fn css_reset_detection() {
        let src = "/* css reset */\nhtml, body, div, span { margin: 0; padding: 0; }";
        let n = name(src).unwrap();
        assert_eq!(n, "css-reset");
    }

    #[test]
    fn noise_selectors_filtered() {
        let src = "#root {\n  height: 100%;\n}\n#app {\n  display: flex;\n}\n.modal-overlay {\n  position: fixed;\n}";
        let n = name(src).unwrap();
        assert!(n.contains("modal-overlay"), "noise IDs filtered: {n}");
    }
}
