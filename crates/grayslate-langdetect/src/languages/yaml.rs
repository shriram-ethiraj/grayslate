...
pub(crate) fn is_likely_yaml(trimmed: &str, _was_sliced: bool) -> bool {
    ...
    // Anti-signal: CSS/SCSS patterns — braces + property: value; lines
    static CSS_SELECTOR: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"^\s*[.#][\w\-]+\s*\{|^\s*(body|html|div|main)\s*\{|^\s*\*\s*\{").unwrap()
    });
    static CSS_DECL: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"^\s*[\w\-]+\s*:\s*[^;]+;\s*$").unwrap()
    });
    let css_selector_count = non_empty.iter().filter(|l| CSS_SELECTOR.is_match(l)).count();
    let css_decl_count = non_empty.iter().filter(|l| CSS_DECL.is_match(l)).count();
    if css_selector_count >= 1 && css_decl_count >= 1 {
        return false;
    }
