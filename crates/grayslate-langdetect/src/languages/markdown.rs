...
/// after a blank line) so embedded code snippets don't trigger disqualifiers
/// against markdown detection.
///
...
fn has_markdown_frontmatter(trimmed: &str) -> bool {
    ...
    // Guard: component files (Svelte, Vue, MDX) with frontmatter aren't markdown.
    ...
    // Guard: pure YAML/data content after the "frontmatter" means this is
    // multi-document YAML, not markdown with frontmatter.
    // Check that post-frontmatter content has at least one unambiguous markdown
    // signal (headings, links, emphasis, code fences — NOT lists, which overlap
    // with YAML list syntax).
    static MD_AFTER_FM: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"(?m)(^#{1,6}\s+\S|\[.+?\]\(.+?\)|\*\*.+?\*\*|^```|^\s*>\s+\S)").unwrap()
    });
    static YAML_DOMINATED: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"(?m)^\s*[a-zA-Z_][\w.\-]*\s*:").unwrap()
    });
    let yaml_lines = YAML_DOMINATED.find_iter(after).count();
    let total_non_blank = after.lines().filter(|l| !l.trim().is_empty()).count();
    // If there are YAML-like lines and no unambiguous markdown signal, it's YAML
    if yaml_lines > 0 && !MD_AFTER_FM.is_match(after) {
        return false;
    }
    // If majority of lines are key:value with no markdown prose, it's YAML
    if total_non_blank > 0 && yaml_lines * 2 >= total_non_blank && !MD_AFTER_FM.is_match(after) {
        return false;
    }
...
pub fn definition() -> LanguageDefinition {
    LanguageDefinition {
        ...
        builtins: &[],
        // ── Family-gated fields ──────────────────────────────
        content_families: &[ContentFamily::Markup, ContentFamily::Prose, ContentFamily::Code, ContentFamily::StructuredData, ContentFamily::Config, ContentFamily::ShellScript],
        anchors: &[
            wp!(r"(?m)^#{1,6}\s+\S", 4),
            wp!(r"(?m)^```", 4),
            wp!(r"\[.+?\]\(.+?\)", 4),
...
mod tests {
    ...
    #[test]
    fn markdown_with_yaml_frontmatter_detected() {
        let src = "---\ntitle: My Post\ndate: 2024-01-01\ntags: [rust, web]\n---\n\n# Introduction\n\nThis is a blog post about Rust.\n\n## Getting Started\n\n- Install Rust\n- Create a project\n";
        assert!(is_likely_markdown(src, false), "Markdown with YAML frontmatter should be detected");
    }

    #[test]
    fn markdown_with_frontmatter_and_code_block() {
        let src = "---\ntitle: Shell Tutorial\n---\n\n# Basic Shell Commands\n\nHere are some examples:\n\n```bash\necho \"Hello World\"\nfor i in 1 2 3; do\n  echo $i\ndone\n```\n\n## Conclusion\n\nThat's it!\n";
        assert!(is_likely_markdown(src, false), "Markdown with frontmatter and code block should be detected");
    }

    #[test]
    fn multi_doc_yaml_not_markdown() {
        // Multi-document YAML with --- separators should NOT be markdown
        let src = "---\napiVersion: v1\nkind: ConfigMap\nmetadata:\n  name: config\n---\napiVersion: v1\nkind: Service\nmetadata:\n  name: my-service\nspec:\n  type: ClusterIP\n";
        assert!(!is_likely_markdown(src, false), "Multi-document YAML should not be markdown");
    }

    #[test]
    fn tiny_ruby_module_not_markdown() {
        let src = "# frozen_string_literal: true\nmodule Jekyll\n  Generator = Class.new(Plugin)\nend";
        assert!(!is_likely_markdown(src, false),
            "Tiny Ruby module should not be markdown");
    }
}
