...
pub fn definition() -> LanguageDefinition {
    LanguageDefinition {
        ...
        hints: &[
            ...
            wp!(r"(?m)^\s*fastcgi_pass\s+", 3),
            wp!(r"(?m)^\s*try_files\s+", 3),
            wp!(r"(?m)^\s*proxy_set_header\s+", 3),
            wp!(r"(?m)^\s*server_name\s+", 3),
            wp!(r"(?m)^\s*rewrite\s+", 2),
        ],
        disqualifiers: &[
            wp!(r"<VirtualHost\s", -5),
            wp!(r"(?m)^\s*(def|class|import)\s+\w+", -4),
        ],
    }
}
