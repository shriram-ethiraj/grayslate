...
impl FamilyResult {
    ...
    /// The top-2 families (for ambiguous cases where we run both).
    pub fn top_two(&self) -> Vec<&FamilyScore> {
        self.scores.iter().take(2).collect()
    }

    /// Whether the classification is confident (top score >> second score).
    pub fn is_confident(&self) -> bool {
        match (self.scores.first(), self.scores.get(1)) {
            (Some(first), Some(second)) => first.confidence - second.confidence > 0.2,
...
mod tests {
    use super::*;
    use crate::detection::features::extract_features;
